<template>
    <div class="peopleList" style="overflow-x: auto">
        ...
        <router-link to="/CreateRoom">
          <el-button type="danger" icon="el-icon-plus" class="addpeople" size='mini'>新建会议室</el-button>
        </router-link>
        <div class="table-box">
          <div class="top">
            <!-- <div class="action">
            ...
            <div class="search">
             <el-input placeholder="请输入关键字" v-model="input5" class="input-with-select">
                 <el-select v-model="select" slot="prepend" placeholder="请选择">
                  <el-option label="会议室名称" value="1"></el-option>
                  <el-option label="会议室管理员" value="2"></el-option>
                  <el-option label="会议室主持人" value="3"></el-option>
                  </el-select>
          ...
          <div>
            <el-table :data="tableData" style="width: 100%">
              ...
              <el-table-column prop="people" label="会议室主持人"></el-table-column>
              <el-table-column prop="time" label="创建时间"></el-table-column>
              <el-table-column prop="creator" label="创建者"></el-table-column>
              <el-table-column label="操作">
                <template slot-scope="scope">
                  <el-button type="text" size="mini" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
                  <el-button size="mini" type="text" @click="handleClose(scope.$index, scope.row)" style="color:red">关闭</el-button>
        ...
        <el-dialog title="详情" width='386px' top='0' :visible.sync="dialogTableVisible">
         ...
         <el-dialog title="" :visible.sync="failed" width="352px" top='45vh' center>
          <span>编辑失败，会议正在进行中</span>
          <span slot="footer" class="dialog-footer">
              <el-button type="danger" @click="failed = false">确 定</el-button>
          </span>
        </el-dialog>
        <!-- 关闭 -->
         <el-dialog title="" :visible.sync="closeDialog" width="352px" top='45vh' center>
          <span>{{closeCause}}</span>
          <span slot="footer" class="dialog-footer">
              <el-button type="danger" @click="closeDialog = false">确 定</el-button>
...
export default {
  data () {
    return {
      ...
      failed: false,
      closeDialog: false,
      closeCause: '确定关闭该例行会议室？'
    }
  },
  methods: {
    handleEdit (index, row) {
      this.failed = true
      this.$router.push('/meetingEdit/' + index)
      console.log(index, row)
    },
    handleClose (index, row) {
      // this.closeCause = '当前会议室正在开会中，确定关闭该会议室？'
      this.closeDialog = true
      console.log(index, row)
    },
    handleCheck (index, row) {
      this.$router.push('/meetingDetail/' + index)
    }
  },
  components: {
    vheader
